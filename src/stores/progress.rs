...
/// Reactive container for chapter progress records.
#[derive(Debug, Clone, Copy)]
pub struct ProgressState {
    /// Map of chapter id -> progress.
...
impl ProgressState {
    ...
    #[must_use]
    pub fn use_ctx() -> Self {
        use_context::<Self>().expect("ProgressState must be provided at the root")
    }

    /// Returns the progress for a chapter, if loaded.
    #[must_use]
    pub fn get(&self, chapter_id: uuid::Uuid) -> Option<Progress> {
        self.map.get().get(&chapter_id).cloned()
    }

    /// Fetch all progress records for chapters in a given book.
    pub async fn fetch_for_book(&self, book_id: uuid::Uuid) -> AppResult<()> {
        ...
        let rows: Vec<ProgressWithBook> = client
            .postgrest()
            .from("reading_progress")
            .select(
                "id, user_id, chapter_id, status, time_spent_seconds, updated_at, \
                 reading_chapters!inner(book_id)",
            )
            .eq("user_id", user_id.to_string())
            .eq("reading_chapters.book_id", book_id.to_string())
            .range(0, 4999)
            .get()
            .await?;
    ...
    pub async fn update_status(
        ...
        let body = serde_json::json!({
            ...
            "time_spent_seconds": existing.as_ref().map(|p| p.time_spent_seconds).unwrap_or(0),
            "updated_at": now_iso(),
        });
        ...
        current.insert(progress.chapter_id, progress.clone());
        self.map.set(current);
        Ok(Some(progress))
    }

    /// Add time to a chapter.
    pub async fn log_time(
        ...
        let client = supabase::supabase()?;
        let existing = self.get(chapter_id);
        let new_total = existing.as_ref().map(|p| p.time_spent_seconds).unwrap_or(0) + seconds;
        let body = serde_json::json!({
            "user_id": user_id,
            "chapter_id": chapter_id,
            "status": existing.as_ref().map(|p| p.status).unwrap_or_default().as_str(),
...
/// Helper row that includes the chapter's `book_id` so we can filter.
