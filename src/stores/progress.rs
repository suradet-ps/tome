//! Per-chapter progress state (module-level singleton).
...
thread_local! {
    static STATE: LazyCell<ProgressState> = LazyCell::new(ProgressState::default);
}
...
impl ProgressState {
    ...
    pub async fn update_status(&self, chapter_id: uuid::Uuid, status: ReadingStatus) -> AppResult<Option<Progress>> {
        ...
        let body = serde_json::json!({
            "user_id": user_id, "chapter_id": chapter_id, "status": status.as_str(),
    ...
    pub async fn log_time(&self, chapter_id: uuid::Uuid, seconds: i32) -> AppResult<Option<Progress>> {
        ...
        let body = serde_json::json!({
            ...
            "time_spent_seconds": new_total, "updated_at": now_iso(),
        });
        let progress: Progress = client.postgrest().from("reading_progress").upsert_one(&body, "user_id,chapter_id").await?;
        let mut current = self.map.get();
        current.insert(progress.chapter_id, progress.clone());
